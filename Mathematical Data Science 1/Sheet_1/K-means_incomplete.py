...
# ]
    ...
    while True:
        ...
        for n in range(N):
            x = data[n, :]
            #########   EDIT HERE START   #########
            # a[n] = None # needs to be the index k for which the distance between x and m_k is minimal
            a[n] = np.argmin([x-m_k for m_k in m]) ##np.argmin([np.dot(x-m_k, x-m_k) for m_k in m])
            print(f"Cluster assigned to {n} data point is {a[n]}")
            #########   EDIT HERE END   #########
        ...
        #########   EDIT HERE START   #########
        for k in range(K):
            # m[k, :] = None # change this to be the updated cluster means
            print("Number of points in cluster", len(m_history[0,k,:]))
            print("revised Cluster center",np.sum(a[k]) )
            # print(np.array([data[i,:] for i, cluster_dp in enumerate(a) if cluster_dp==k]).sum(axis=0))
            # print(sum([data[i,:] for i, cluster_dp in enumerate(a) if cluster_dp==k]))
            print(sum([data[i,:] for i, cluster_dp in enumerate(a) if cluster_dp==k])/len(m_history[0,k,:])) ## a = [0,1,1,0,1....n times]
            # break
            if (np.array(a)==k).sum() == 0:
                continue
            m[k, :] = sum([data[i,:] for i, cluster_dp in enumerate(a) if cluster_dp==k])/(np.array(a)==k).sum() ## len(m_history[0,k,:])


            # m_history[0,k,:].shape(1)
        #########   EDIT HERE END   #########
...
if __name__ == "__main__":
    ...
    if option == 1:
        ...
        import pandas as pd        
        #########   EDIT HERE START   #########
        df = pd.read_csv("faithful.csv") # construct a pandas dataframe from the csv-file "faithful.csv"
        print(df) # see what it looks like
        data = df[["eruptions", "waiting"]].to_numpy() # extract the data matrix from the dataframe df
        #########   EDIT HERE END   #########
