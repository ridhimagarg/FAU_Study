...
PR_TOOLBOX_DIR = abspath(__file__)
...
class FeatureSpace(QtWidgets.QWidget):
    ...
    def __init__(self, widget, statusbar, dock):
        ...
        self.setMouseTracking(
            True)  # fires MouseMoveEvents even if no mouse button is pressed

        self.coordinateSystem = CoordinateSystem(self, w, h, -5.1, 5.1, -5.1,
                                                 5.1)
    ...
    def open(self):
        filename, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Open File', join(PR_TOOLBOX_DIR, '..', 'data'), 'Data sets (*)')

        if isfile(filename):
            if filename.lower().endswith('.csv') or filename.lower().endswith('.data'):
                self.samples.clear_samples()
            ...
            if filename.lower().endswith('.p'):
                try:
                    with open(filename, 'rb') as f:
                        rtn = pickle.load(f)
                        self.gaussians, self.samples = rtn

                    self.__filename = filename
                    self.__dir = QtCore.QFileInfo(
                    ...
                    QtWidgets.QMessageBox.warning(
                        self, 'Error',
                        "Error reading the feature space from {0}: {1}".format(
                            filename,
    ...
    def saveAs(self):
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, 'Save File', self.__dir, 'Python Pickle file (*.p)')
    ...
    def exportFile(self):
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, 'Save File', join(PR_TOOLBOX_DIR, '..', 'data'),
            'CSV (*.csv);;ASCII (*.txt);; all files (*.*)')
        ...
        if filename:
            try:
                with open(filename, 'w') as f:
                    ...
                    for sample in samples:
                        f.write("{0:.10f},{1:.10f},{2:d}\n".format(
                            sample[0], sample[1], int(sample[2])))

            except Exception:
                QtWidgets.QMessageBox.warning(
                    self, 'Error',
                    "Error exporting the feature space to {0}".format(
    ...
    def exportImage(self):
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, 'Save File', join(PR_TOOLBOX_DIR, '..', 'data'),
            'Portable Network Graphics (*.png);;JPEG (*.jpg);;Windows Bitmap (*.bmp);;all files (*.*)'
        )
        ...
        if filename:
            try:
                ...
                QtWidgets.QMessageBox.warning(
                    self, 'Error',
                    "Error saving the feature space as image to file {0}".
                    format(filename), QtWidgets.QMessageBox.Ok,
                    QtWidgets.QMessageBox.Ok)

    def importFile(self, filename=None):
        if not filename:
            filename, _ = QtWidgets.QFileDialog.getOpenFileName(
                self, 'Open File', join(PR_TOOLBOX_DIR, '..', 'data'),
                'CSV (*.csv);;ASCII (*.txt);;all files(*.*)')
        ...
        if filename:
            ...
            try:
                ...
                if i > 0:
                    msg = "Error importing feature space from {0} in line {1}: {2}".format(
                        filename, i, line)
                else:
                    msg = "Error importing feature space from {0}".format(
                        filename)
    ...
    def mousePressEvent(self, ev):
        if self.__action == self.ACTION_COORDINATE_SYSTEM:
            ...
            sx, sy = self.coordinateSystem.screen2world(
                self.__mouse_start_x, self.__mouse_start_y)
            ...
            self.__mouse_start_x = ev.pos().x()
            self.__mouse_start_y = ev.pos().y()
            self.__sx, self.__sy = self.coordinateSystem.screen2world(
                self.__mouse_start_x, self.__mouse_start_y)
            ...
            for gaussian in self.gaussians:
                (handled,
                 op) = gaussian.mousePressEvent(ev, self,
                                                self.coordinateSystem)
                if handled:
                    break
            if handled:
                if op is not None:
                    if not self.__widget.runFeatureSpaceComputations(
                            initialize=True):
                        self.repaint()
                    self.__widget.operationStack.add(op)
            else:
    ...
    def mouseMoveEvent(self, ev):
        if self.__action == self.ACTION_COORDINATE_SYSTEM:
            ...
            self.__ex, self.__ey = self.coordinateSystem.screen2world(
                self.__mouse_end_x, self.__mouse_end_y)
            ...
            if self.__mouse_pressed:
                ...
                self.__statusbar.showMessage(
                    "{0:.2f}, {1:.2f} to {2:.2f}, {3:.2f}; width: {4:.2f}, height: {5:.2f}"
                    .format(self.__sx, self.__sy, self.__ex, self.__ey, w, h))
                self.repaint()
            else:
                self.__statusbar.showMessage("{0:.2f}, {1:.2f}".format(
                    self.__ex, self.__ey))
        elif self.__action == self.ACTION_MODIFY_GAUSSIAN:
            handled = False
            for gaussian in self.gaussians:
    ...
    def mouseReleaseEvent(self, ev):
        if self.__action == self.ACTION_COORDINATE_SYSTEM:
            ...
            if not (self.__widget.densityEstimator is None):
                # resizing requires histogram re-estimation
                repaint = self.__widget.runFeatureSpaceComputations(
                    initialize=True)
            else:
                repaint = self.__widget.runFeatureSpaceComputations(
                    initialize=False)
            ...
            if self.__mouse_dragging:
                ...
                self.__ex = int(self.__ex * 100 + 0.5) / 100.0
                self.__ey = int(self.__ey * 100 + 0.5) / 100.0
                (created, op) = self.createGaussian(self.__sx, self.__sy,
                                                    self.__ex, self.__ey)
                if created:
                    if not self.__widget.runFeatureSpaceComputations(
                            initialize=True):
            ...
            for gaussian in self.gaussians:
                (handled,
                 op) = gaussian.mouseReleaseEvent(ev, self.coordinateSystem)
                if handled:
                    break
            if handled:
                if op is None:
            ...
            if self.__mouse_dragging:
                ...
                if self.samples.move_samples:
                    ...
                    self.samples.selectArea(self.__sx, self.__sy, self.__ex,
                                            self.__ey)
                    count, labels = self.samples.countSamplesInSelectedArea()
                    if count == 0:
                        self.__statusbar.showMessage('No samples selected')
                    elif count == 1:
                        self.__statusbar.showMessage('1 sample selected')
                    elif len(labels) == 1:
                        self.__statusbar.showMessage(
                            '{0} samples of the same class selected'.format(
                                count))
                    else:
                        self.__statusbar.showMessage(
                            '{0} samples of {1} classes selected'.format(
                                count, len(labels)))
                ...
                if not (self.__widget.densityEstimator is None):
                    # resizing requires histogram re-estimation
                    repaintDone = self.__widget.runFeatureSpaceComputations(
                        initialize=True)
                else:
                    repaintDone = self.__widget.runFeatureSpaceComputations(
                        initialize=False)
    ...
    def convertGaussian(self, gaussianId, redoOperation=False):
        ...
        if not redoOperation:
            op = Operation(self, "FeatureSpace.convertGaussian",
                           (gaussianId, m))
            self.__widget.operationStack.add(op)
    ...
    def deleteGaussian(self, gaussianId, confirmation=True):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            if confirmation:
                ...
                op = Operation(self, "FeatureSpace.deleteGaussian",
                               (gaussianId))
                # self.__widget.operationStack.add(op)
                return (True, op)
    ...
    def createGaussian(self, x1, y1, x2, y2):
        ...
        propertyForm = GaussProperties(classId, numSamples, mean1, mean2,
                                       cov11, cov22, cov12, True, self)
        ...
        if result == QtWidgets.QDialog.Accepted:
            ...
            gaussian = Gaussian2D(
                self, classId, numSamples, numpy.array([mean1, mean2]),
                numpy.array([[cov11, cov12], [cov12, cov22]]), isIncluded,
                None, self.__statusbar)
            self.gaussians.append(gaussian)
