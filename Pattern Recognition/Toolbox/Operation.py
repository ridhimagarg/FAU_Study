...
class Operation(object):
    ...
    def undo(self):
        if self.operation == "CoordinateSystem.move":
            ...
            self.featurespace.samples.insertSamples(removedSamples, indices,
                                                    area)
        elif self.operation == "Samples2D.assignClassLabel":
            (updateList, _, oldLabels, area) = self.args
            self.featurespace.samples.assignNewClassLabel(
                updateList, oldLabels, area)
        elif self.operation == "Samples2D.moveSamples":
            (updateList, dx, dy, _, oldArea) = self.args
            self.featurespace.samples.moveSamples(updateList, -dx, -dy,
                                                  oldArea)
            ...
            (classId, numSamples, mean1, mean2, cov11, cov22, cov12,
             isIncluded) = oldParams
            gaussian.setProperties(classId, numSamples, mean1, mean2, cov11,
            ...
            print("error: unknown operation '{0}' cannot be undone".format(
                self.operation))
    ...
    def redo(self):
        if self.operation == "CoordinateSystem.move":
            ...
            print("error: unknown operation '{0}' cannot be redone".format(
                self.operation))
