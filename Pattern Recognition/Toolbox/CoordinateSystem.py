...
class CoordinateSystem(object):
    ...
    def setLimits(self,
        ...
        if self.__equalAxis:
            self.__pixelsXPerUnit = self.__pixelsYPerUnit = min(
                self.__pixelsXPerUnit, self.__pixelsYPerUnit)
    ...
    def setWidgetSize(self, width, height):
        ...
        self.setLimits(self.__preferred_xmin, self.__preferred_xmax,
                       self.__preferred_ymin, self.__preferred_ymax, self.__dx,
                       self.__dy, self.__zoom, self.__equalAxis)

    def world2screen(self, worldX, worldY):
        screenX = int((worldX - self.__xmin) * self.__width /
                      (self.__xmax - self.__xmin))
        screenY = int((worldY - self.__ymin) * self.__height /
    ...
    def getTicsRange(self, min_value, max_value):
        tics_min = int(min_value) if min_value < 0 else int(
            math.ceil(min_value))
        tics_max = int(max_value) if max_value > 0 else int(
    ...
    def mouseReleaseEvent(self, ev):
        if self.__mouse_dragging:
            self.__mouse_dragging = False
            sx, sy = self.screen2world(self.__mouse_start_x,
                                       self.__mouse_start_y)
            ex, ey = self.screen2world(ev.pos().x(), ev.pos().y())
            dx, dy = ex - sx, ey - sy
            self.setLimits(self.__preferred_xmin, self.__preferred_xmax,
                           self.__preferred_ymin, self.__preferred_ymax,
                           self.__dx - dx, self.__dy - dy, self.__zoom,
                           self.__equalAxis)
            return Operation(self.featurespace, "CoordinateSystem.move",
                             (dx, dy))
