...
class ClusteringParameters(QtWidgets.QDialog):
    ...
    def checkParameters(self, tab):
        if tab == 0:  # k-means
            if not self.testParameter(
                    self.getKMeansK,
                    'The number of clusters has to be an integer number!'):
                return False
            ...
            if k > Parameters.NUMBER_SUPPORTED_CLASSES:
                self.errorMsg(
                    'Currently a maximum number of {0} clusters is supported!'.
                    format(Parameters.NUMBER_SUPPORTED_CLASSES))
                return False
        elif tab == 1:  # GMM
            if not self.testParameter(
                    self.getGmmK,
    ...
    def setTab(self, tab):
        if tab >= 0:
            self.buttons.button(
                QtWidgets.QDialogButtonBox.Ok).setText("Compute clusters")
