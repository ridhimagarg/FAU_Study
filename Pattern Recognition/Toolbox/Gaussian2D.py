...
class Gaussian2D(object):
    ...
    def myInit(self):
        ...
        self.__samples = self.mvnrdn(len(self.__raw_samples[0]), self.__mean,
                                     self.__covmat, self.__raw_samples)
    ...
    def __getstate__(self):
        return (self.__classId, self.__num_samples, self.__mean, self.__covmat,
                self.__raw_samples, self.__excluded)

    def __setstate__(self, state):
        (self.__classId, self.__num_samples, self.__mean, self.__covmat,
         self.__raw_samples, self.__excluded) = state
        self.myInit()
    ...
    def __createContextMenu(self, widget):
        ...
        group = QtWidgets.QActionGroup(widget)
        group.setExclusive(True)
        self.classActions = list()
        for i in range(Parameters.NUMBER_SUPPORTED_CLASSES):
            a = QtWidgets.QAction("Class {0}".format(i + 1), widget)
            action = group.addAction(a)
    ...
    def setNumSamples(self, numSamples):
        ...
        if numSamples <= len(self.__samples[0]):
            ...
            new_samples = self.mvnrdn(n, self.__mean, self.__covmat,
                                      new_raw_samples)
            self.__raw_samples = numpy.append(self.__raw_samples,
                                              new_raw_samples,
                                              axis=1)
    ...
    def mvnrdn(self,
               ...
               values=None):
        d = len(mean)
        assert len(covmat) == len(
            covmat[0]), "The covariance matrix must be quadratic"
        assert d == len(
            covmat
        ), "The dimensions of the mean vector and the covariance matrix do not match"
        L = numpy.linalg.cholesky(covmat)
        if values is None:
            values = numpy.zeros((d, n))
            for i in range(d):
                values[i] = self.normrdn(n, 0, 1)
        return numpy.dot(L, values) + numpy.matlib.repmat(
            mean.reshape((d, 1)), 1, n)

    def setMean(self, x, y):
        self.__mean = numpy.array([x, y])
        self.__samples = self.mvnrdn(len(self.__samples[0]), self.__mean,
                                     self.__covmat, self.__raw_samples)
        self.__mean_estimated = numpy.mean(self.__samples, 1)
    ...
    def paint(self, qp, coordinateSystem):
        ...
        if not self.__excluded:
            ...
            for (x1, x2) in self.__samples[:, 0:display_max_samples].T:
                ...
                if self.__pointstyle == self.CROSS:
                    qp.drawLine(x1 - self.__pointsize, x2,
                                x1 + self.__pointsize, x2)
                    qp.drawLine(x1, x2 - self.__pointsize, x1,
                                x2 + self.__pointsize)
                elif self.__pointstyle == self.SQUARE:
                    qp.drawRect(x1 - self.__pointsize, x2 - self.__pointsize,
                                w, w)
                elif self.__pointstyle == self.CIRCLE:
                    qp.drawEllipse(x1 - self.__pointsize,
                                   x2 - self.__pointsize, w, w)
                else:
                    qp.drawPoint(x1, x2)
        ...
        # draw center
        self.__cx, self.__cy = coordinateSystem.world2screen(
        ...
        if self.__action == self.ACTION_CHANGE_COVMAT:
            self.__sigma1 = math.sqrt(self.__covmat[0, 0])
            self.__sigma2 = math.sqrt(self.__covmat[1, 1])
            x1, y1 = coordinateSystem.world2screen(
                self.__mean[0] - self.__sigma1, self.__mean[1] - self.__sigma2)
            x2, y2 = coordinateSystem.world2screen(
                self.__mean[0] + self.__sigma1, self.__mean[1] + self.__sigma2)
            w = x2 - x1
            h = y2 - y1
    ...
    def mouseNearCenter(self, mx, my):
        return self.__action == self.ACTION_MOVE and math.pow(
            mx - self.__cx, 2) + math.pow(my - self.__cy, 2) < 25

    def mouseNearRatioButton(self, mx, my):
        return self.__action == self.ACTION_CHANGE_COVMAT and math.pow(
            mx - self.__rx, 2) + math.pow(my - self.__ry, 2) < 25
    ...
    def mouseNearSigma1Boundary(self, mx, my, coordinateSystem):
        if self.__action == self.ACTION_CHANGE_COVMAT:
            ...
            if math.fabs(
                    mx - x1
            ) < 2 and my > y1 and my < y2:  # left edge of the bounding box
                return True
            if math.fabs(
                    mx - x2
            ) < 2 and my > y1 and my < y2:  # right edge of the bounding box
                return True
        return False
    ...
    def mouseNearSigma2Boundary(self, mx, my, coordinateSystem):
        if self.__action == self.ACTION_CHANGE_COVMAT:
            ...
            if math.fabs(
                    my - y1
            ) < 2 and mx > x1 and mx < x2:  # top edge of the bounding box
                return True
            if math.fabs(
                    my - y2
            ) < 2 and mx > x1 and mx < x2:  # bottom edge of the bounding box
                return True
        return False
    ...
    def mouseMoveEvent(self, ev, widget, coordinateSystem):
        ...
        if self.__mouse_pressed:
            ...
            if self.__action == self.ACTION_MOVE and self.__mouse_operation == self.MOVING_CENTER:
                self.setMean(ex, ey)
                self.__statusbar.showMessage("Center at ({0}, {1})".format(
                    ex, ey))
                widget.repaint()
                ...
                self.__statusbar.showMessage(
                    "Correlation = {0:0.2f}".format(cov12))
                widget.repaint()
    ...
    def mouseReleaseEvent(self, ev, coordinateSystem):
        ...
        if ev.button(
        ) == QtCore.Qt.LeftButton and not self.__mouse_operation == self.NONE:
            mx = ev.pos().x()
            my = ev.pos().y()
            ...
            if self.__mouse_dragging:
                ...
                if self.__action == self.ACTION_MOVE and self.__mouse_operation == self.MOVING_CENTER:
                    ...
                    self.setMean(ex, ey)
                    self.__statusbar.showMessage('')
                    op = Operation(self.__widget, "Gaussian2D.setMean",
                                   (self.__id, newParams, oldParams))
                    return (True, op)
                elif self.__action == self.ACTION_CHANGE_COVMAT and self.__mouse_operation == self.MOVING_SIGMA1:
                    sigma = int(math.fabs(ex - self.__mean[0]) * 100 +
                                0.5) / 100.0
                    self.setSigma(0, sigma)
                    self.__statusbar.showMessage('')
                    (_, _, _, _, oldSigma, _, _, _) = self.__oldProperties
                    op = Operation(self.__widget, "Gaussian2D.setSigma",
                                   (self.__id, 0, sigma, oldSigma))
                    return (True, op)
                elif self.__action == self.ACTION_CHANGE_COVMAT and self.__mouse_operation == self.MOVING_SIGMA2:
                    sigma = int(math.fabs(ey - self.__mean[1]) * 100 +
                                0.5) / 100.0
                    self.setSigma(1, sigma)
                    self.__statusbar.showMessage('')
                    (_, _, _, _, _, oldSigma, _, _) = self.__oldProperties
                    op = Operation(self.__widget, "Gaussian2D.setSigma",
                                   (self.__id, 1, sigma, oldSigma))
                    return (True, op)
                elif self.__action == self.ACTION_CHANGE_COVMAT and self.__mouse_operation == self.MOVING_COR:
                    cov12 = (ex - self.__mean[0]) / self.__sigma1
                    self.setCov12(cov12)
                    ...
                    op = Operation(self.__widget, "Gaussian2D.setCov12",
                                   (self.__id, cov12, oldCov12))
                    return (True, op)
            else:
                if math.pow(mx - self.__cx, 2) + math.pow(my - self.__cy,
                                                          2) < 25:
    ...
    def editProperties(self):
        propertyForm = GaussProperties(self.__classId, self.__num_samples,
                                       ...
                                       self.__covmat[1, 1],
                                       self.__covmat[0, 1],
                                       not self.__excluded, self.__widget)
        result = propertyForm.exec_()

        if result == QtWidgets.QDialog.Accepted:
            (classId, numSamples, mean1, mean2, cov11, cov22, cov12,
             isIncluded) = propertyForm.getProperties()
            ...
            oldParams = (self.__classId, self.__num_samples, self.__mean[0],
                         self.__mean[1], self.__covmat[0, 0],
                         self.__covmat[1, 1], self.__covmat[0, 1],
                         not self.__excluded)
            newParams = (classId, numSamples, mean1, mean2, cov11, cov22,
                         cov12, isIncluded)
            if (oldParams != newParams):
                self.setProperties(classId, numSamples, mean1, mean2, cov11,
                                   cov22, cov12, isIncluded)

                op = Operation(self.__widget, "Gaussian2D.editProperties",
                               (self.__id, newParams, oldParams))
    ...
    def setProperties(self, classId, numSamples, mean1, mean2, cov11, cov22,
                      cov12, isIncluded):
        self.__classId = classId
        self.setColors()
    ...
    def getProperties(self):
        return (self.__classId, self.__num_samples, self.__mean[0],
                self.__mean[1], self.__covmat[0, 0], self.__covmat[1, 1],
                self.__covmat[0, 1], not self.__excluded)
