...
class ClassifierParameters(QtWidgets.QDialog):
    def __init__(self, parent):
        ...
        LogRegNumIterationsLabel = QtWidgets.QLabel(
        ...
        GmmNumIterationsLabel = QtWidgets.QLabel(
            'Maximum number of iterations')
        ...
        self.KNNAlgorithmCombobox.addItem('NN')
        self.KNNAlgorithmCombobox.currentIndexChanged.connect(
            self.onKNNAlgorithmChanged)
        ...
        PerceptronNumIterationsLabel = QtWidgets.QLabel(
            'Maximum number of iterations')
        ...
        # MLP properties
        MLPHiddenLayersLabel = QtWidgets.QLabel(
            'Number of neurons in each hidden layer')
        MLPActivationLabel = QtWidgets.QLabel(
        ...
        self.SVMAlgorithmCombobox.currentIndexChanged.connect(
            self.onSVMAlgorithmChanged)
        ...
        DecisionTreeMinSamplesSplitLabel = QtWidgets.QLabel(
            'Minimum samples required to split internal node')
        DecisionTreeMinSamplesLeafLabel = QtWidgets.QLabel(
            'Minimum samples required at leaf node')
        DecisionTreeMinWeightedFractionLeafLabel = QtWidgets.QLabel(
            'Minimum weighted fraction at leaf node')
        DecisionTreeMaxLeafNodesLabel = QtWidgets.QLabel('Maximum leaf nodes')
        DecisionTreeNumTrialsPerSplitLabel = QtWidgets.QLabel(
            'Number of trials to split a node')
        self.DecisionTreeAlgorithmCombobox = QtWidgets.QComboBox()
        self.DecisionTreeAlgorithmCombobox.addItem(
            'DecisionTreeClassifier (scikit-learn)')
        self.DecisionTreeAlgorithmCombobox.addItem('Decision Tree')
        self.DecisionTreeAlgorithmCombobox.currentIndexChanged.connect(
            self.onDecisionTreeAlgorithmChanged)
        ...
        self.DecisionTreeMaxDepthCombobox.addItems(
            [str(x) for x in range(1, 10)])
        self.DecisionTreeMaxDepthCombobox.setEditable(True)
        self.DecisionTreeMaxDepthCombobox.setInsertPolicy(
        ...
        self.DecisionTreeMinWeightedFractionLeafEdit = QtWidgets.QLineEdit(
            '0.')
        self.DecisionTreeMaxLeafNodesCombobox = QtWidgets.QComboBox()
        self.DecisionTreeMaxLeafNodesCombobox.addItems(
            ['None', '10', '20', '30', '40', '50'])
        self.DecisionTreeMaxLeafNodesCombobox.setEditable(True)
        self.DecisionTreeMaxLeafNodesCombobox.setInsertPolicy(
            QtWidgets.QComboBox.InsertAlphabetically)
        self.DecisionTreeMaxLeafNodesCombobox.currentIndexChanged.connect(
            self.onDecisionTreeMaxLeafNodesChanged)
        ...
        DecisionTreeGrid.addWidget(DecisionTreeMinWeightedFractionLeafLabel, 6,
                                   0)
        DecisionTreeGrid.addWidget(
            self.DecisionTreeMinWeightedFractionLeafEdit, 6, 1)
        ...
        DecisionTreeGrid.addWidget(self.DecisionTreeNumTrialsPerSplitEdit, 8,
                                   1)
        ...
        RandomForestMinSamplesSplitLabel = QtWidgets.QLabel(
            'Minimum samples required to split internal node')
        RandomForestMinSamplesLeafLabel = QtWidgets.QLabel(
            'Minimum samples required at leaf node')
        RandomForestMinWeightedFractionLeafLabel = QtWidgets.QLabel(
            'Minimum weighted fraction at leaf node')
        RandomForestMaxLeafNodesLabel = QtWidgets.QLabel('Maximum leaf nodes')
        RandomForestNumTrialsPerSplitLabel = QtWidgets.QLabel(
            'Number of trials to split a node')
        self.RandomForestAlgorithmCombobox = QtWidgets.QComboBox()
        self.RandomForestAlgorithmCombobox.addItem(
            'RandomForestClassifier (scikit-learn)')
        self.RandomForestAlgorithmCombobox.addItem('Random Forest')
        self.RandomForestAlgorithmCombobox.currentIndexChanged.connect(
            self.onRandomForestAlgorithmChanged)
        ...
        self.RandomForestMaxDepthCombobox.addItems(
            [str(x) for x in range(1, 10)])
        self.RandomForestMaxDepthCombobox.setEditable(True)
        self.RandomForestMaxDepthCombobox.setInsertPolicy(
            QtWidgets.QComboBox.InsertAlphabetically)
        self.RandomForestMinSamplesSplitEdit = QtWidgets.QLineEdit('2')
        self.RandomForestMinSamplesLeafEdit = QtWidgets.QLineEdit('1')
        self.RandomForestMinWeightedFractionLeafEdit = QtWidgets.QLineEdit(
            '0.')
        self.RandomForestMaxLeafNodesCombobox = QtWidgets.QComboBox()
        self.RandomForestMaxLeafNodesCombobox.addItems(
            ['None', '10', '20', '30', '40', '50'])
        self.RandomForestMaxLeafNodesCombobox.setEditable(True)
        self.RandomForestMaxLeafNodesCombobox.setInsertPolicy(
            QtWidgets.QComboBox.InsertAlphabetically)
        self.RandomForestMaxLeafNodesCombobox.currentIndexChanged.connect(
            self.onRandomForestMaxLeafNodesChanged)
        ...
        RandomForestGrid.addWidget(RandomForestMinWeightedFractionLeafLabel, 6,
                                   0)
        RandomForestGrid.addWidget(
            self.RandomForestMinWeightedFractionLeafEdit, 6, 1)
        ...
        RandomForestGrid.addWidget(self.RandomForestNumTrialsPerSplitEdit, 8,
                                   1)
    ...
    def checkParameters(self, tab):
        if tab == 0:  # Linear Logistic Regression parameters
            if not self.testParameter(
                    self.getLogRegMaxNumIterations,
                    'The maximum number of iterations has to be an integer number!'
            ):
                return False

            if not self.testParameter(
                    self.getLogRegLearningRate,
                    'The learning rate has to be a floating point number!'):
                return False

            maxNumIter = self.getLogRegMaxNumIterations()
            if maxNumIter < 1:
                self.errorMsg(
                    'The maximum number of iterations has to be positive!')
                return False
            ...
            if not self.testParameter(
                    self.getGmmNumComponentsPerClass,
                    'The number of components has to be a positive integer number!'
            ):
                return False

            if not self.testParameter(
                    self.getGmmMaxNumIterations,
                    'The maximum number of iterations has to be an integer number!'
            ):
                return False
            ...
            if len(k) != self.__numClasses:
                self.errorMsg(
                    'Wrong specification of the number of GMM components for each class!'
                )
                return False
            ...
            if not self.testParameter(
                    self.getKNNNumberOfNeighbors,
                    'The number of neighbors has to be an integer number!'):
                return False
            ...
            if not self.testParameter(
                    self.getPerceptronMaxNumIterations,
                    'The maximum number of iterations has to be an integer number!'
            ):
                return False

            if not self.testParameter(
                    self.getPerceptronLearningRate,
                    'The learning rate has to be a floating point number!'):
                return False
            ...
            if not self.testParameter(
                    self.getMLPAlpha,
                    'The Alpha parameter has to be a floating point number!'):
                return False
            ...
            if not self.testParameter(
                    self.getRandomForestNumTrees,
                    'The number of trees in the forest has to be an integer number!'
            ):
                return False

            if not self.testParameter(
                    self.getRandomForestMaxDepth,
                    'The maximum depth of the tree has to be None or an integer number!'
            ):
                return False

            if not self.testParameter(
                    self.getRandomForestMaxLeafNodes,
                    'The maximum number of leaf nodes has to be None or an integer number!'
            ):
                return False

            if not self.testParameter(
                    self.getRandomForestMinSamplesSplit,
                    'The minimum number of samples required to split an internal node has to be an integer number!'
            ):
                return False

            if not self.testParameter(
                    self.getRandomForestMinSamplesLeaf,
                    'The minimum number of samples required to be at a leaf node has to be an integer number!'
            ):
                return False

            if not self.testParameter(
                    self.getRandomForestMinWeightedFractionLeaf,
                    'The minimum weighted fraction of the input samples required to be at a leaf node has to be a real number!'
            ):
                return False

            if not self.testParameter(
                    self.getRandomForestNumTrialsPerSplit,
                    'The number of trials to split a node has to be an integer number!'
            ):
                return False
    ...
    def getParameters(self):
        ...
        params[
            'perceptron_maxnumiterations'] = self.PerceptronNumIterationsEdit.text(
            )
        params[
            'perceptron_learningrate'] = self.PerceptronLearningRateEdit.text(
            )
        params[
            'perceptron_batchmode'] = self.PerceptronBatchModeCombobox.currentIndex(
            )
        params['mlp_hiddenlayers'] = self.MLPHiddenLayersEdit.text()
        params[
            'mlp_activation'] = self.MLPActivationFunctionCombobox.currentIndex(
            )
        ...
        params[
            'decision_tree_algo'] = self.DecisionTreeAlgorithmCombobox.currentIndex(
            )
        params[
            'decision_tree_criterion'] = self.DecisionTreeCriterionCombobox.currentIndex(
            )
        params[
            'decision_tree_splitter'] = self.DecisionTreeSplitterCombobox.currentIndex(
            )
        params[
            'decision_tree_max_depth'] = self.DecisionTreeMaxDepthCombobox.currentIndex(
            )
        params[
            'decision_tree_min_samples_split'] = self.DecisionTreeMinSamplesSplitEdit.text(
            )
        params[
            'decision_tree_min_samples_leaf'] = self.DecisionTreeMinSamplesLeafEdit.text(
            )
        params[
            'decision_tree_min_weighted_fraction_leaf'] = self.DecisionTreeMinWeightedFractionLeafEdit.text(
            )
        params[
            'decision_tree_max_leaf_nodes'] = self.DecisionTreeMaxDepthCombobox.currentIndex(
            )
        params[
            'decision_tree_num_trials_per_split'] = self.DecisionTreeNumTrialsPerSplitEdit.text(
            )
        params[
            'random_forest_algo'] = self.RandomForestAlgorithmCombobox.currentIndex(
            )
        params['random_forest_numtrees'] = self.RandomForestNumTreesEdit.text()
        params[
            'random_forest_criterion'] = self.RandomForestCriterionCombobox.currentIndex(
            )
        params[
            'random_forest_max_depth'] = self.RandomForestMaxDepthCombobox.currentIndex(
            )
        params[
            'random_forest_min_samples_split'] = self.RandomForestMinSamplesSplitEdit.text(
            )
        params[
            'random_forest_min_samples_leaf'] = self.RandomForestMinSamplesLeafEdit.text(
            )
        params[
            'random_forest_min_weighted_fraction_leaf'] = self.RandomForestMinWeightedFractionLeafEdit.text(
            )
        params[
            'random_forest_max_leaf_nodes'] = self.RandomForestMaxDepthCombobox.currentIndex(
            )
        params[
            'random_forest_num_trials_per_split'] = self.RandomForestNumTrialsPerSplitEdit.text(
            )
        return params
    ...
    def restoreParameters(self):
        ...
        self.PerceptronNumIterationsEdit.setText(
            params['perceptron_maxnumiterations'])
        self.PerceptronLearningRateEdit.setText(
            params['perceptron_learningrate'])
        self.PerceptronBatchModeCombobox.setCurrentIndex(
            params['perceptron_batchmode'])
        self.MLPHiddenLayersEdit.setText(params['mlp_hiddenlayers'])
        self.MLPActivationFunctionCombobox.setCurrentIndex(
            params['mlp_activation'])
        self.MLPAlgorithmCombobox.setCurrentIndex(params['mlp_algo'])
        self.MLPAlphaEdit.setText(params['mlp_alpha'])
        self.MLPLearningRateCombobox.setCurrentIndex(
            params['mlp_learningrate'])
        ...
        self.DecisionTreeAlgorithmCombobox.setCurrentIndex(
            params['decision_tree_algo'])
        self.DecisionTreeCriterionCombobox.setCurrentIndex(
            params['decision_tree_criterion'])
        self.DecisionTreeSplitterCombobox.setCurrentIndex(
            params['decision_tree_splitter'])
        self.DecisionTreeMaxDepthCombobox.setCurrentIndex(
            params['decision_tree_max_depth'])
        self.DecisionTreeMinSamplesSplitEdit.setText(
            params['decision_tree_min_samples_split'])
        self.DecisionTreeMinSamplesLeafEdit.setText(
            params['decision_tree_min_samples_leaf'])
        self.DecisionTreeMinWeightedFractionLeafEdit.setText(
            params['decision_tree_min_weighted_fraction_leaf'])
        self.DecisionTreeMaxDepthCombobox.setCurrentIndex(
            params['decision_tree_max_leaf_nodes'])
        self.DecisionTreeNumTrialsPerSplitEdit.setText(
            params['decision_tree_num_trials_per_split'])
        self.RandomForestAlgorithmCombobox.setCurrentIndex(
            params['random_forest_algo'])
        self.RandomForestNumTreesEdit.setText(params['random_forest_numtrees'])
        self.RandomForestCriterionCombobox.setCurrentIndex(
            params['random_forest_criterion'])
        self.RandomForestMaxDepthCombobox.setCurrentIndex(
            params['random_forest_max_depth'])
        self.RandomForestMinSamplesSplitEdit.setText(
            params['random_forest_min_samples_split'])
        self.RandomForestMinSamplesLeafEdit.setText(
            params['random_forest_min_samples_leaf'])
        self.RandomForestMinWeightedFractionLeafEdit.setText(
            params['random_forest_min_weighted_fraction_leaf'])
        self.RandomForestMaxDepthCombobox.setCurrentIndex(
            params['random_forest_max_leaf_nodes'])
        self.RandomForestNumTrialsPerSplitEdit.setText(
            params['random_forest_num_trials_per_split'])
    ...
    def onDecisionTreeAlgorithmChanged(self, idx):
        if idx == 0:  # DecisionTreeClassifier (scikit-learn)
            ...
            if self.DecisionTreeMaxDepthCombobox.currentText().lower(
            ) == 'none':
    ...
    def onRandomForestAlgorithmChanged(self, idx):
        if idx == 0:  # RandomForestClassifier (scikit-learn)
            ...
            if self.RandomForestMaxDepthCombobox.currentText().lower(
            ) == 'none':
    ...
    def setTab(self, tab):
        if tab >= 0:
            self.buttons.button(
                QtWidgets.QDialogButtonBox.Ok).setText("Classify")
