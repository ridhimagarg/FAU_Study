...
class Regression(object):
    ...
    def initialize(self):
        ...
        if self.__regressor == self.LinearRegression:
            ...
            _ = self.__parameters.getSVRAlgorithm(
            )  # there's just one algorithm right now
            ...
            self.__reg = svm.SVR(kernel=kernel,
                                 degree=degree,
                                 gamma=gamma,
                                 coef0=coef0,
                                 C=C,
                                 epsilon=epsilon)
            self.__reg.fit(self.__samples[:, 0].reshape(-1, 1),
                           self.__samples[:, 1])

        elif self.__regressor == self.RegressionTree:
            algorithm = self.__parameters.getRegressionTreeAlgorithm()
            _ = self.__parameters.getRegressionTreeCriterion(
            )  # currently there's just one quality criterion implemented
            ...
            if algorithm == 'sklearn':
                self.__reg = tree.DecisionTreeRegressor(
                    splitter=splitter,
                    max_features=1,
                    max_depth=maxDepth,
            ...
            _ = self.__parameters.getRegressionForestCriterion(
            )  # currently there's just one quality criterion implemented
            ...
            if algorithm == 'sklearn':
                self.__reg = ensemble.RandomForestRegressor(
                ...
                self.__reg = RegressionForest(numTrees, maxDepth,
                                              minSamplesLeaf, trials)
            self.__reg.fit(self.__samples[:, 0].reshape(-1, 1),
                           self.__samples[:, 1])
    ...
    def paint(self, qp):
        if self.__reg:
            ...
            if self.__regressor == self.LinearRegression:
            ...
            elif (self.__regressor == self.SVR) or (
                    self.__regressor == self.RegressionTree) or (
                        self.__regressor == self.RegressionForest):
                ...
                for x, y in zip(X[:, 0], Z):
                    x, y = self.__featurespace.coordinateSystem.world2screen(
                        x, y)
