...
class DensityEstimationParameters(QtWidgets.QDialog):
    def __init__(self, parent):
        ...
        # Histogram tab
        histogramBinsXLabel = QtWidgets.QLabel(
            'Number of bins per unit in x direction')
        histogramBinsYLabel = QtWidgets.QLabel(
            'Number of bins per unit in y direction')
        ...
        sphereNeighborsLabel = QtWidgets.QLabel(
            'Number of neighbors inside the sphere')
        ...
        kernelDensityEstimationKernelLabel = QtWidgets.QLabel(
            'Kernel function')
        ...
        self.kernelDensityEstimationAlgorithmCombobox.addItem(
            'KernelDensity (scikit-learn)')
        self.kernelDensityEstimationAlgorithmCombobox.addItem(
            'Kernel Density Estimation')
        self.kernelDensityEstimationAlgorithmCombobox.currentIndexChanged.connect(
            self.onKernelDensityEstimationAlgorithmChanged)
        ...
        kernelDensityEstimationGrid.addWidget(
            kernelDensityEstimationAlgorithmLabel, 0, 0)
        kernelDensityEstimationGrid.addWidget(
            self.kernelDensityEstimationAlgorithmCombobox, 0, 1)
        kernelDensityEstimationGrid.addWidget(
            kernelDensityEstimationKernelLabel, 1, 0)
        kernelDensityEstimationGrid.addWidget(
            self.kernelDensityEstimationKernelCombobox, 1, 1)
        kernelDensityEstimationGrid.addWidget(
            kernelDensityEstimationBandwidthLabel, 2, 0)
        kernelDensityEstimationGrid.addWidget(
            self.kernelDensityEstimationBandwidthEdit, 2, 1)
        kernelDensityEstimationGrid.addWidget(
            kernelDensityEstimationColormapLabel, 3, 0)
        kernelDensityEstimationGrid.addWidget(
            self.kernelDensityEstimationColormapCombobox, 3, 1)
        ...
        self.kernelDensityEstimationTab.setLayout(
            kernelDensityEstimationLayout)
    ...
    def checkParameters(self, tab):
        if tab == 0:  # Histogram parameters
            if not self.testParameter(
                    self.getHistogramBinsX,
                    'The number of bins per unit in x direction has to be a floating point number!'
            ):
                return False

            if not self.testParameter(
                    self.getHistogramBinsY,
                    'The number of bins per unit in y direction has to be a floating point number!'
            ):
                return False
            ...
            if n <= 0:
                self.errorMsg(
                    'The number of bins per unit in x direction has to be positive!'
                )
                return False
            ...
            if n <= 0:
                self.errorMsg(
                    'The number of bins per unit in y direction has to be positive!'
                )
                return False

        elif tab == 1:  # Sphere Density Estimation parameters
            if not self.testParameter(
                    self.getSphereDensityEstimationNeighbors,
            ...
            if n <= 0:
                self.errorMsg(
                    'The number of neighbors inside the sphere has to be positive!'
                )
                return False

        elif tab == 2:  # Kernel Density Estimation parameters
            if not self.testParameter(
                    self.getKernelDensityEstimationBandwidth,
                    'The bandwidth of the kernel function has to be a floating point number!'
            ):
                return False
            ...
            if w <= 0:
                self.errorMsg(
                    'The bandwidth of the kernel function has to be positive!')
                return False
    ...
    def getParameters(self):
        ...
        params[
            'kde_algorithm'] = self.kernelDensityEstimationAlgorithmCombobox.currentIndex(
            )
        params[
            'kde_kernel'] = self.kernelDensityEstimationKernelCombobox.currentIndex(
            )
        params[
            'kde_bandwidth'] = self.kernelDensityEstimationBandwidthEdit.text(
            )
        params[
            'kde_colormap'] = self.kernelDensityEstimationColormapCombobox.currentIndex(
            )
        return params
    ...
    def restoreParameters(self):
        ...
        self.histogramColormapCombobox.setCurrentIndex(
            params['histo_colormap'])
        ...
        self.kernelDensityEstimationAlgorithmCombobox.setCurrentIndex(
            params['kde_algorithm'])
        self.kernelDensityEstimationKernelCombobox.setCurrentIndex(
            params['kde_kernel'])
        self.kernelDensityEstimationBandwidthEdit.setText(
            params['kde_bandwidth'])
        self.kernelDensityEstimationColormapCombobox.setCurrentIndex(
            params['kde_colormap'])
    ...
    def setTab(self, tab):
        if tab >= 0:
            self.buttons.button(
                QtWidgets.QDialogButtonBox.Ok).setText("Estimate density")
