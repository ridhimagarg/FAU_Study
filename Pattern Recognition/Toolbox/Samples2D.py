...
class Samples2D(object):
    ...
    def paint(self, qp, coordinateSystem):
        ...
        for (x1, x2, classId) in self.__samples.T:
            ...
            if self.__pointstyle == self.CROSS:
                qp.drawLine(x1 - self.__pointsize, x2, x1 + self.__pointsize,
                            x2)
                ...
                qp.drawEllipse(x1 - self.__pointsize, x2 - self.__pointsize, w,
                               w)
            else:
                qp.drawPoint(x1, x2)
        ...
        if self.__areaSelected:
            ...
            x1, y1 = coordinateSystem.world2screen(self.__area_x1,
                                                   self.__area_y1)
            x2, y2 = coordinateSystem.world2screen(self.__area_x2,
                                                   self.__area_y2)
    ...
    def assignNewClassLabelInSelectedArea(self, classId):
        if self.__areaSelected:
            ...
            if len(updateList) > 0:
                ...
                op = Operation(self.__featurespace,
                               "Samples2D.assignClassLabel",
                               (updateList, classId, oldLabels, area))
                self.__featurespace.getWidget().operationStack.add(op)
    ...
    def moveSamplesInSelectedArea(self, dx, dy):
        if self.__areaSelected:
            ...
            m = len(updateList)
            if m > 0:
                oldArea = (self.__area_x1, self.__area_y1, self.__area_x2,
                           self.__area_y2)
                newArea = (self.__area_x1 + dx, self.__area_y1 + dy,
                           self.__area_x2 + dx, self.__area_y2 + dy)
                self.moveSamples(updateList, dx, dy, newArea)
                op = Operation(self.__featurespace, "Samples2D.moveSamples",
                               (updateList, dx, dy, newArea, oldArea))
                self.__featurespace.getWidget().operationStack.add(op)
            else:
                self.__area_x1 += dx
                self.__area_y1 += dy
