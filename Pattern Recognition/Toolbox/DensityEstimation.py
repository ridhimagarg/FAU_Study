...
class DensityEstimation(object):
    ...
    def initialize(self):
        ...
        if self.__estimator == self.Histogram:
            ...
            self.__est = HistogramDensityEstimation(
                binsX, binsY, self.__featurespace.coordinateSystem)
            ...
            self.__est = SphereDensityEstimation(
                k, self.__featurespace.coordinateSystem)
            self.__est.fit(self.__samples)
            ...
            if algorithm == "sklearn":
                self.__est = neighbors.kde.KernelDensity(kernel=kernel,
                                                         bandwidth=bandwidth)
            else:
